...
@section('content')
    @include('backend.elements.content_action', ['title' => $title, 'action' => 'backend.elements.actions.ggadwords.ggadwords_list'])
    @include('backend.elements.content_search', ['search' => 'backend.elements.searchs.ggadwords.ggadwords_search'])
    <div class="row">
        <div class="col-xl-12">
                                    ...
                                    @if(!empty($reports) && count($reports) > 0)
                                        @foreach($reports as $key => $item)
                                            <tr>
                                                ...
                                                <td data-label="Tiêu đề"><a href="@php echo $urlHelper::admin('ggadwords', 'edit')."?id=$item->report_id" @endphp">{{ $item->report_name }}</a></td>
                                                <td data-label="Xem báo cáo">
                                                    <a href="@php echo $urlHelper::admin('ggadwords', 'report')."?id=$item->report_id" @endphp">
                                                ...
                                                <td data-label="Id"><a href="@php echo $urlHelper::admin('ggadwords', 'edit')."?id=$item->report_id" @endphp">{{ $item->report_id }}</a></td>
                                            </tr>
                                        @endforeach
                                        @else
                                            <tr>
