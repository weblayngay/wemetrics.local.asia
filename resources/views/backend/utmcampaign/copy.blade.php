<?php
    ...
    $utmcampaign = !empty($data['utmcampaign']) ? $data['utmcampaign'] : null;
...
@section('content')
    @include('backend.elements.content_action', ['title' => $title, 'action' => 'backend.elements.actions.utmcampaign.utmcampaign_copy'])
    <form method="POST" id="admin-form" action="{{app('UrlHelper')::admin('utmcampaign', 'duplicate')}}" enctype="multipart/form-data">
        <input type="hidden" name="action_type">
        <input type="hidden" name="avatarId" value="{{ $avatarId }}">
        <input type="hidden" name="id" value="{{$utmcampaign->id}}">
        @csrf
        <div class="row">
            <div class="col-xl-12">
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="row">
                                                    <div class="col-md-2">
                                                        <fieldset class="form-group">
                                                            <label for="name">{{__('Tên utm campaign')}} <span class="red">*</span></label>
                                                        </fieldset>
                                                        <hr>
                                                    </div>
                                                    <div class="col-md-10">
                                                        <fieldset class="form-group">
                                                            <input type="text" name="name" class="form-control" placeholder="Tên utm campaign" value="{{ $utmcampaign->name }}" required="">
                                                        </fieldset>
                                                    </div>
                                                </div>  
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-6">
                                                        <fieldset class="form-group">
                                                            <div class="form-check form-check-inline">
                                                                <input class="form-check-input" type="radio" name="status" value="activated" @if($utmcampaign->status == 'activated') checked @endif>
                                                                <label class="form-check-label" >{{__('Có')}}</label>
                                                            </div>
                                                            <div class="form-check form-check-inline">
                                                                <input class="form-check-input" type="radio" name="status" value="inactive" @if($utmcampaign->status == 'inactive') checked @endif>
