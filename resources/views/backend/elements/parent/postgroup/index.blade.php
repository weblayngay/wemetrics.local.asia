@if(count($parents) > 0)
    @foreach ($parents as $key => $parents_item)
        <option value="{{($parents_item->postgroup_id)}}" {{ ($parents_item->postgroup_id == $parentId) ? 'selected' : '' }}>
            {{ $verticalBar . str_pad('', $node, $separate, STR_PAD_RIGHT) . $parents_item->postgroup_name}}
        </option>
