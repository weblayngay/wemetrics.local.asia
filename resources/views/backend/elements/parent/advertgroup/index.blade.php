@if(count($parents) > 0)
    @foreach ($parents as $key => $parents_item)
        <option value="{{($parents_item->adgroup_id)}}" {{ ($parents_item->adgroup_id == $parentId) ? 'selected' : '' }}>
            {{ $verticalBar . str_pad('', $node, $separate, STR_PAD_RIGHT) . $parents_item->adgroup_name}}
        </option>
