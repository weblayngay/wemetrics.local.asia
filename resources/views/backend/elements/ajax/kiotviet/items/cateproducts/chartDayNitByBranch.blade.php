<script type="text/javascript">
    function chartDayNitByBranch(data) {
        // Load Data
        var series = [];
        var labels = [];
        ...
        var title_text = 'Thống kê dây nịt theo cửa hàng';
        ...
        jQuery.each(data.daynitByBranch, function( i, item ) {
          if(i < 5)
          {
            series.push(parseInt(data.daynitByBranch[i].quantity));
            labels.push(data.daynitByBranch[i].branchName);
          }
        });
        ...
        var chart = new ApexCharts(document.querySelector("#chartDayNitByBranch"), options);
        chart.render();
    }
</script>