<script type="text/javascript">
    function chartPhuKienByBranch(data) {
        // Load Data
        var series = [];
        var labels = [];
        ...
        var title_text = 'Thống kê phụ kiện theo cửa hàng';
        ...
        jQuery.each(data.phukienByBranch, function( i, item ) {
          if(i < 5)
          {
            series.push(parseInt(data.phukienByBranch[i].quantity));
            labels.push(data.phukienByBranch[i].branchName);
          }
        });
        ...
        var chart = new ApexCharts(document.querySelector("#chartPhuKienByBranch"), options);
        chart.render();
    }
</script>