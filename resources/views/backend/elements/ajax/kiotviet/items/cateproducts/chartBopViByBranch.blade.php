<script type="text/javascript">
    function chartBopViByBranch(data) {
        // Load Data
        var series = [];
        var labels = [];
        ...
        var title_text = 'Thống kê bóp ví theo cửa hàng';
        ...
        jQuery.each(data.bopviByBranch, function( i, item ) {
          if(i < 5)
          {
            series.push(parseInt(data.bopviByBranch[i].quantity));
            labels.push(data.bopviByBranch[i].branchName);
          }
        });
        ...
        var chart = new ApexCharts(document.querySelector("#chartBopViByBranch"), options);
        chart.render();
    }
</script>