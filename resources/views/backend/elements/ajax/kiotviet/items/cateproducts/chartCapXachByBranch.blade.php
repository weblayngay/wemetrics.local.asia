<script type="text/javascript">
    function chartCapXachByBranch(data) {
        // Load Data
        var series = [];
        var labels = [];
        ...
        var title_text = 'Thống kê cặp xách theo cửa hàng';
        ...
        jQuery.each(data.capxachByBranch, function( i, item ) {
          if(i < 5)
          {
            series.push(parseInt(data.capxachByBranch[i].quantity));
            labels.push(data.capxachByBranch[i].branchName);
          }
        });
        ...
        var chart = new ApexCharts(document.querySelector("#chartCapXachByBranch"), options);
        chart.render();
    }
</script>