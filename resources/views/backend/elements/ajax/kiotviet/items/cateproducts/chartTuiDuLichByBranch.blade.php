<script type="text/javascript">
    function chartTuiDuLichByBranch(data) {
        // Load Data
        var series = [];
        var labels = [];
        ...
        var title_text = 'Thống kê túi du lịch theo cửa hàng';
        ...
        jQuery.each(data.tuidulichByBranch, function( i, item ) {
          if(i < 5)
          {
            series.push(parseInt(data.tuidulichByBranch[i].quantity));
            labels.push(data.tuidulichByBranch[i].branchName);
          }
        });
        ...
        var chart = new ApexCharts(document.querySelector("#chartTuiDuLichByBranch"), options);
        chart.render();
    }
</script>