<script type="text/javascript">
    function chartTuiDeoByBranch(data) {
        // Load Data
        var series = [];
        var labels = [];
        var branchId = [];
        ...
        var title_text = 'Thống kê túi đeo theo cửa hàng';
        ...
        jQuery.each(data.tuideoByBranch, function( i, item ) {
          if(i < 5)
          {
            series.push(parseInt(data.tuideoByBranch[i].quantity));
            labels.push(data.tuideoByBranch[i].branchName);
          }
        });
        ...
        var chart = new ApexCharts(document.querySelector("#chartTuiDeoByBranch"), options);
        chart.render();
    }
</script>