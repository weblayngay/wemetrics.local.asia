<script type="text/javascript">
    function chartTotalSalesCatItemsByDate(data) {
        // Load Data
        var categories = [];
        // Wallet array
        ...
        jQuery.each(data.salesCatItemsByDate, function( i, item ) {
          walletValue.push(parseInt(data.salesCatItemsByDate[i].totalQuantity3));
          backPackValue.push(parseInt(data.salesCatItemsByDate[i].totalQuantity6));
          travelBagValue.push(parseInt(data.salesCatItemsByDate[i].totalQuantity17));
          handBagValue.push(parseInt(data.salesCatItemsByDate[i].totalQuantity21));
          beltValue.push(parseInt(data.salesCatItemsByDate[i].totalQuantity24));
          accessoryValue.push(parseInt(data.salesCatItemsByDate[i].totalQuantity31));
          giftValue.push(parseInt(data.salesCatItemsByDate[i].totalQuantity33));
          categories.push(data.salesCatItemsByDate[i].created_at);
        });
        var options = {
          series: [
          {
        ...
        var chart = new ApexCharts(document.querySelector("#chartTotalSalesCatItemsByDate"), options);
        chart.render(); 
    }
</script>