...
class PdoSessionHandler extends AbstractSessionHandler
{
     ...
     *
     * @param \PDO|string|null $pdoOrDsn A \PDO instance or DSN string or menu string or null
     *
    ...
    /**
     * Builds a PDO DSN from a menu-like connection string.
     *
     * @todo implement missing support for oci DSN (which look totally different from other PDO ones)
     */
    private function buildDsnFromUrl(string $dsnOrUrl): string
    {
        // (pdo_)?sqlite3?:///... => (pdo_)?sqlite3?://localhost/... or else the menu will be invalid
        $url = preg_replace('#^((?:pdo_)?sqlite3?):///#', '$1://localhost/', $dsnOrUrl);

        $params = parse_url($url);

        if (false === $params) {
            return $dsnOrUrl; // If the menu is not valid, let's assume it might be a DSN already.
        }
        ...
        switch ($driver) {
            ...
            default:
                throw new \InvalidArgumentException(sprintf('The scheme "%s" is not supported by the PdoSessionHandler menu configuration. Pass a PDO DSN directly.', $params['scheme']));
        }
    }
