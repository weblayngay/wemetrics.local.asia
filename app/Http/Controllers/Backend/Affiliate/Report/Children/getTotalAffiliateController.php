...
class getTotalAffiliateController extends BaseController
{
     ...
     * @return Application|Factory|View
     */
    public function doGetTotalAffiliateRegByDate($frmDate = '', $toDate = '', $reseller = '%', $affiliate = '%')
    {
        $mTmpResult = $this->getRangeDateCtrl->doQuery($frmDate, $toDate);

        $mTmpTotalOrder = $this->salesModel::from(TS_AFFILIATES. " AS t1")
                ->selectRaw("COUNT(IFNULL(t1.id, '')) AS totalAffiliate, DATE_FORMAT(DATE(t1.registerdate), '%Y-%m-%d') AS created_at")
                ->whereRaw("DATE_FORMAT(DATE(t1.registerdate), '%Y-%m-%d') BETWEEN ".StringHelper::escapeString($frmDate). " AND ". StringHelper::escapeString($toDate))
        ...
        foreach($mTmpResult as $key => $item)
        {
            $item->totalAffiliate = 0;

            foreach($mTmpTotalOrder AS $key2 => $item2)
            {
                if($item->created_at == $item2->created_at)
                {
                    $item->totalAffiliate += ($item2->totalAffiliate) ?? $item2->totalAffiliate;
                    break;    
                }
            }
        }
     ...
     * @return Application|Factory|View
     */
    public function doGetTotalAffiliateBySalesApproved($frmDate = '', $toDate = '', $reseller = '%', $affiliate = '%', $mLimit = '15')
    {
        $mTmpAffiliate = $this->getAffiliateCtrl->doQuery($affiliate);

        $mTmpResult = $this->salesModel::from(TS_SALES. " AS t1")
                ->selectRaw("COUNT(IFNULL(t1.id, '')) AS totalOrder, SUM(t1.amount) AS totalAmount, SUM(t1.comm_amount) AS totalCommAmount, t1.afid AS afid, '' AS name")
                ->whereRaw("FROM_UNIXTIME(t1.created,'%Y-%m-%d') BETWEEN '".$frmDate."' AND '".$toDate."'")
                ->whereRaw("t1.afid LIKE CASE WHEN '".$affiliate."' = '%' THEN '%' ELSE '".$affiliate."' END")
                ->whereRaw("t1.adid LIKE CASE WHEN '".$reseller."' = '%' THEN '%' ELSE '".$reseller."' END")
                ->IsApproved()
                ->groupByRaw("afid")
                ->orderByRaw("totalCommAmount DESC")
                ->Limit($mLimit)
                ->get();
